...
/* ===== カタログ（itemsは定数、generated_atはレスポンス時に生成） ===== */
const CATALOG_ITEMS = [{
        id: "spyxr",
        ...
        image_url: "https://www.usj.co.jp/tridiondata/usj/ja/jp/files/images/gds-images/usj-gds-spy-family-2025-b.jpg",
        area: "ハリウッド・エリア",
        active: true,
    },
    {
        ...
        image_url: "https://www.usj.co.jp/tridiondata/usj/ja/jp/files/images/gds-images/usj-gds-hollywood-dream-the-ride-b.jpg",
        area: "ハリウッド・エリア",
        active: true,
    },
];
...
function parseWaitStats(html, source) {
    ...
    const mm = (label) => {
        const m = aroundNorm.match(new RegExp(label + "\\s*:?\\s*([0-9]{1,4})\\s*分[^0-9:]*([0-9]{1,2}:[0-9]{2})", "i"));
        return m ? { v: parseInt(m[1], 10), t: m[2] } : { v: num(label), t: null };
    };

    const currentMatch = aroundNorm.match(/(リアルタイム|現在|最新)\s*:?\s*([0-9]{1,4})\s*分/i) ? .[2] || null;
